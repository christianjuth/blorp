...
declare module 'one' {
  export namespace OneRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(stack)` | `/(stack)/communities` | `/(tabs)` | `/(tabs)/` | `/(tabs)/(stack)` | `/(tabs)/(stack)/communities` | `/(tabs)/chat` | `/(tabs)/communities` | `/(tabs)/create` | `/(tabs)/inbox` | `/_sitemap` | `/auth` | `/chat` | `/communities` | `/create` | `/inbox` | `/md`
