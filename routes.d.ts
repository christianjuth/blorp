...
declare module 'one' {
  export namespace OneRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(communities)` | `/(communities)/communities` | `/(tabs)` | `/(tabs)/` | `/(tabs)/(communities)` | `/(tabs)/(communities)/communities` | `/(tabs)/chat` | `/(tabs)/communities` | `/(tabs)/create` | `/(tabs)/settings` | `/_sitemap` | `/auth` | `/chat` | `/communities` | `/create` | `/md` | `/settings`
