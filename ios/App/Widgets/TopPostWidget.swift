...
struct TopPostProvider: AppIntentTimelineProvider {
  ...
  func snapshot(for configuration: TopPostConfigurationAppIntent, in context: Context) async
    -> TopPostEntry
  {
    TopPostEntry(
      date: Date(), title: "Lenny the giant loggerhead turtle heads back to the sea",
      community: "upliftingnews@lemmy.world", apId: "")
  }

  func timeline(for configuration: TopPostConfigurationAppIntent, in context: Context) async
    -> Timeline<TopPostEntry>
  {
    let date = Date()
