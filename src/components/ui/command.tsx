...
function CommandInput({
  ...
  return (
    ...
    >
      {searchIcon && <Search className="size-5 shrink-0 opacity-50" />}
      <CommandPrimitive.Input
        data-slot="command-input"
        className={cn(
          "placeholder:text-muted-foreground flex h-10 w-full rounded-lg bg-transparent py-3 outline-hidden disabled:cursor-not-allowed disabled:opacity-50",
          className,
        )}
        {...props}
      />
      {loading && (
        <Spinner className="size-5 shrink-0 opacity-50 animate-spin" />
      )}
      {endAdornment}
    </div>
  );
...
function CommandShortcut({
  ...
  return (
    <span
      ...
      className={cn(
        "text-muted-foreground ml-auto text-xs tracking-widest bg-muted px-1.5 py-0.5 rounded-sm",
        className,
      )}
      {...props}
    />
