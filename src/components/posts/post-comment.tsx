...
function Byline({
  ...
  const personView = useProfilesStore(
    (s) => s.profiles[getCachePrefixer()(actorId)]?.data,
  );
  const slug = personView?.person ? createSlug(personView.person) : null;
  return (
    <summary
      className={cn("flex flex-row gap-1.5 items-center py-px", className)}
    >
      <Avatar className="w-6 h-6">
        <AvatarImage src={personView?.person.avatar} />
        <AvatarFallback className="text-xs">
          {personView?.person.name?.substring(0, 1).toUpperCase()}{" "}
        </AvatarFallback>
      </Avatar>
